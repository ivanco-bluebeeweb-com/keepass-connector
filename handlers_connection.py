"""Connection lifecycle for KeePass Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from keepass_client import KeePassClient
...
@chat.function(
    "connect_keepass",
    "Connect KeePass account via credentials.",
    action_type="write",
    chain_callable=True,
    event="keepass-connector.connect_keepass",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_keepass(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect KeePass Connector."""
    client = KeePassClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "KeePass Account",
...
@chat.function(
    "list_connections",
    "List connected KeePass accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_keepass",
    "Disconnect KeePass account.",
    action_type="write",
    chain_callable=True,
    event="keepass-connector.disconnect_keepass",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_keepass(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
